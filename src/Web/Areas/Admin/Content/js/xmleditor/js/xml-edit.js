...
//定义一个初始化类
function xmleditorInit() {

}

//封装一个静态初始化方法
xmleditorInit.Init = function () {
...
//加载xml
xmleditorInit.SetXml = function (xmlStr) {
    xmleditor.text = genXML();
    xmleditor.editor.setValue(xmlStr);
    loadXMLFile(JXON.stringToXml(xmlStr));
}
//获取xml
xmleditorInit.GetXml = function () {
    if ($('#btn-text').hasClass("selected")) {
        return xmleditor.editor.getValue();
    }
    else {
        xmleditor.text = genXML();
        xmleditor.editor.setValue(xmleditor.text);
