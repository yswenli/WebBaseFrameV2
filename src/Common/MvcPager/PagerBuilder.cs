...
namespace MvcPager
{
    ...
    internal class PagerBuilder
    {
        ...
        private const string JqCheckScript = "if(typeof(jQuery)==\"undefined\"){alert(\"未检测到jQuery脚本库，ASP.NET MvcPager无法实现jQuery Ajax分页！\");} $('.grid-body').height(document.documentElement.clientHeight - $('body > .panel-content').outerHeight() + $('.grid-body').height());";
        ...
        {
            //若要生成url的页索引小于1或大于总页数或等于当前页索引时，无需生成分页导航链接pageIndex < 1 || 
            if (pageIndex > _totalPageCount || pageIndex == _pageIndex)
                return null;

            var routeValues = _routeValues ?? new RouteValueDictionary();

            // 设置Route Value中的分页导航Url参数值
            if (pageIndex == 0) //pageIndex为0时生成适用于脚本的导航链接
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            if (_pagerOptions.ShowPageIndexBox)
            {
              //  sb.Append(BuildGoToPageSection(ref pagerScript));
                sb.Replace("</strong><strong", "</strong>" + BuildGoToPageSection(ref pagerScript) + "<strong");
            }
            else
        ...
        private string BuildGoToPageSection(ref string pagerScript)
        {
            ...
            if (_pagerOptions.PageIndexBoxType == PageIndexBoxType.DropDownList)
            {
                ...
                piBuilder.AppendFormat(
                    " <span>第</span><input type=\"text\" name=\"pageId\" class=\"text\" size=\"2\" id=\"{0}\" value=\"{1}\" onkeydown=\"_MvcPager_Keydown(event)\"{2}/><span>页 共" + _totalPageCount.ToString() + "页</span>",
        ...
        {
            if (item.Disabled)
                return CreateWrappedPagerElement(item, String.Format("<a href='#'>{0}</a>", item.Text));
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            //pager item link
            string url = GenerateUrl(item.PageIndex);
            return CreateWrappedPagerElement(item, String.Format("<a href=\"{0}\">{1}</a>", url, item.Text));
        }
